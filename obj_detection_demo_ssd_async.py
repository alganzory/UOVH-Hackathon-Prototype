...
chosenInput = False #to perform a cbeck on whether the user uploaded a file or not
cap = None
...
def main():
    ...
    fileButton = tk.Button (inputFrame, width= 20, text = 'Choose a file', border = 5, command = UploadAction)
    fileButton.grid(row = 0, column = 1, columnspan =3)
    cameraButton = tk.Button (inputFrame,width= 20, text= 'Use Camera',border = 5, command = CameraAction)
                    ...
                    for obj in output_postprocessor(exec_net.requests[cur_request_id].output_blobs):
                        ...
                        if obj[2] > args.prob_threshold:
                            ...
                            cv2.putText(frame, det_label + ' ' + str(round(obj[2] * 100, 1)) + ' %', (xmin, ymin - 7),
                                        cv2.FONT_HERSHEY_DUPLEX, 0.8, (225,225,225), 1)
